...
namespace DriftDemo
{
    public class DemoCar : IDemo
    {
        ...
        public void Init(Space space)
        {
            ...
            // Left ramp polygon
            var leftRampVerts = new Vector2[]
            {
                new Vector2(-10.24f, 0),
                new Vector2(-2, 0),
                new Vector2(-2, 1),
                new Vector2(-9.44f, 4),
                new Vector2(-10.24f, 4)
            };
            staticBody.AddShape(new ShapePoly(leftRampVerts));

            // Right ramp polygon
            var rightRampVerts = new Vector2[]
            {
                new Vector2(2, 0),
                new Vector2(10.24f, 0),
                new Vector2(10.24f, 4),
            ...
            for (int i = 0; i < 10; i++)
            {
                var body = new Body(Body.BodyType.Dynamic, new Vector2(-1.8f + i * 0.4f, 0.9f));
                ...
                if (i == 0)
                {
                    ...
                    var joint = new RevoluteJoint(staticBody, body, new Vector2(-2, 0.9f));
                ...
                else
                {
                    ...
                    var joint = new RevoluteJoint(prevBody!, body, new Vector2(-2 + i * 0.4f, 0.9f));
                    joint.CollideConnected = false;
            ...
            var lastJoint = new RevoluteJoint(prevBody!, staticBody, new Vector2(2, 0.9f));
            lastJoint.CollideConnected = false;
            space.AddJoint(lastJoint);

            // Create car body
            var carBody = new Body(Body.BodyType.Dynamic, new Vector2(-8, 5));
            var carVerts = new Vector2[]
            {
            ...
            // Create wheel 1 (rear)
            var wheel1Body = new Body(Body.BodyType.Dynamic, new Vector2(-8.5f, 4.9f));
            ...
            var wheelJoint1 = new WheelJoint(carBody, wheel1Body, new Vector2(-8.5f, 5), new Vector2(-8.5f, 4.9f));
            ...
            var wheel2Body = new Body(Body.BodyType.Dynamic, new Vector2(-7.5f, 4.9f));
            ...
            var wheelJoint2 = new WheelJoint(carBody, wheel2Body, new Vector2(-7.5f, 5), new Vector2(-7.5f, 4.9f));
            ...
            for (int i = 0; i < 3; i++)
            {
                antennaBodies[i] = new Body(Body.BodyType.Dynamic, new Vector2(-8.55f, 5.94f + 0.2f * i));
                ...
                if (i == 0)
                {
                    ...
                    var antennaJoint = new WeldJoint(carBody, antennaBodies[0], new Vector2(-8.55f, 5.84f + 0.2f * i));
                ...
                else
                {
                    ...
                    var antennaJoint = new WeldJoint(antennaBodies[i - 1], antennaBodies[i], new Vector2(-8.55f, 5.84f + 0.2f * i));
                    antennaJoint.CollideConnected = false;
                    antennaJoint.SetSpringFrequencyHz(30);
                    antennaJoint.SetSpringDampingRatio(0.1f);
                    space.AddJoint(antennaJoint);
